...
struct Extractor final : osmium::handler::Handler
{
    ...
    Extractor(Datacontainer *container) { c = container; }
    ...
    void way(const osmium::Way &way)
    {
        ...
        if (usable && way.nodes().size() > 1 && (forward || reverse))
        {
            ...
            c->key_value_pairs.emplace_back(c->addstring("_way_id"),
                                            c->addstring(std::to_string(way.id()).c_str()));
            BOOST_ASSERT(c->key_value_pairs.size() < std::numeric_limits<std::uint32_t>::max());
            ...
            std::for_each(
                 ...
                 reverse](boost::tuple<const osmium::NodeRef, const osmium::NodeRef> pair)
                {
                    try
                    {
                        ...
                        {
                            internal_a_id =
                                static_cast<internal_nodeid_t>(c->used_nodes_list.size());
                        ...
                        if (forward)
                        {
                            c->pair_tagset_map.emplace(std::make_pair(internal_a_id, internal_b_id),
                                                       tagset_id);
                        }
                        if (reverse)
                        {
                            c->pair_tagset_map.emplace(std::make_pair(internal_b_id, internal_a_id),
                                                       tagset_id);
                        }
                    }
...
int main(int argc, char **argv) try
{
    ...
    // GET Request Handler
    listener.support(
        web::http::methods::GET, [&data](const auto &request)
        {
                ...
                external_nodeids.size() < 2)
            {
                ...
                // boost::geometry::strategy::distance::haversine<double> const
                // haversine(6372795.0);
                     ...
                     lonIdx += 2, latIdx += 2)
                {
                    ...
                    data.rtree->query(boost::geometry::index::nearest(pt, 1),
            ...
            else
            {
                ...
                std::for_each(external_nodeids.begin(), external_nodeids.end(),
                              [&data, &external_nodeids](const external_nodeid_t n)
                              {
            ...
            std::for_each(
                boost::make_zip_iterator(
                    boost::make_tuple(internal_nodeids.cbegin(), internal_nodeids.cbegin() + 1)),
                boost::make_zip_iterator(
                    boost::make_tuple(internal_nodeids.cend() - 1, internal_nodeids.cend())),
                [&data, &tagset_offset_list, &tagset_indexes,
                 &tagset_list](boost::tuple<const internal_nodeid_t, const internal_nodeid_t> pair)
                {
                    // If our pair exists in the pair_tagset_map
                    const auto tagset_id =
                        data.pair_tagset_map.find(std::make_pair(pair.get<0>(), pair.get<1>()));
                    if (tagset_id != data.pair_tagset_map.end())
                    {
                        if (tagset_indexes.find(tagset_id->second) == tagset_indexes.end())
                        {
                            ...
                            tagset_indexes[tagset_id->second] =
                                static_cast<int>(tagset_list.size() - 1);
                        }
                        tagset_offset_list.push_back(
                            tagset_indexes.find(tagset_id->second)->second);
                    }
                    else
                    {
            ...
            for (const auto &tagset_id : tagset_list)
            {
                ...
                for (auto i = tagset.first; i <= tagset.second; i++)
                {
                    ...
                    tags[data.getstring(key_value_pair.first)] =
                        web::json::value(data.getstring(key_value_pair.second));
                }
                waydata[idx++] = tags;
            }
